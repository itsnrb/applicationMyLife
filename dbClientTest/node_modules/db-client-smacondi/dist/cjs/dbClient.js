...
var _dbOptimisticLock = _interopRequireDefault(require("./errors/dbOptimisticLock.js"));
var _lodash = _interopRequireDefault(require("lodash"));
var _utils = require("./support/utils.js");
var _notFound = _interopRequireDefault(require("./errors/notFound.js"));
...
class DbClient {
  ...
  constructor(connectionString, readTimeout = READ_TIMEOUT, writeTimeout = WRITE_TIMEOUT) {
    ...
    _logger.default.info(`Set default query timeout in dbClient to ${this.readTimeout} ms.`);
    this.writeTimeout = this._getTimeout(writeTimeout, WRITE_TIMEOUT);
    _logger.default.info(`Set default write timeout in dbClient to ${this.writeTimeout} ms.`);
  }
   ...
   */
  async connect() {
    this.client = await _mongodb.MongoClient.connect(this.connectionString, {
      maxPoolSize: 100,
  ...
  async findOneOrThrow(dbName, collectionName, query, options) {
    ...
    throw new _notFound.default('No record found');
  }
  ...
  async findOneById(dbName, collectionName, documentId, options = undefined) {
    ...
    let idAsObjectId;
    try {
      idAsObjectId = new _mongodb.ObjectId(documentId);
    } catch (error) {
      _logger.default.error(`Invalid object id given to findOneById for db ${dbName}, collection ${collectionName}, documentId ${documentId}. Error: ${error.message}`);
      throw error;
    }
    const collection = await this._getCollection(dbName, collectionName);
  ...
  async aggregate(dbName, collectionName, pipeline, options) {
    try {
      ...
      _logger.default.error(`Error during aggregation: ${error}. DB: ${dbName}, COLLECTION: ${collectionName}, PIPELINE: ${JSON.stringify(pipeline)}`);
      throw error;
    }
  }
  ...
  async replaceOneById(dbName, collectionName, document, options) {
    ...
    const documentId = (0, _utils.getMongoDbObjectId)(document._id);
  ...
  async replaceOneByIdWithVersionCheck(dbName, collectionName, document, options) {
    ...
    throw new _dbOptimisticLock.default('Version mismatch or document does not exist anymore.');
  }
  ...
  async deleteOneById(dbName, collectionName, documentId, options) {
    ...
    return await collection.deleteOne({
      _id: new _mongodb.ObjectId(documentId)
  ...
  async inTransaction(callback, sessionOptions = {}, transactionOptions = undefined) {
    ...
    _logger.default.info('Start transaction session');
    try {
      ...
      _logger.default.info('Transaction session finished');
    }
  }
